...
module.exports = class Prysmo extends EventEmitter{
    ...
    close(){
        ...
        // Throw if Prysmo was already closed.
        if(!this.server)
            throw new Error('Cannot close a closed Prysmo');
    ...
    /*------------------------------------------------------------------------o\
        Register key-value pairs of @spec as endpoint-listener. If @prefix,
        prepend it to all endpoint names.
    \o------------------------------------------------------------------------*/
    entity(spec, prefix){
        assert.equal(typeof spec, 'object');
        let p = prefix ? String(prefix) : '';
        for(let k in spec)
            this.endpoint(p + k, spec[k]);
    }
